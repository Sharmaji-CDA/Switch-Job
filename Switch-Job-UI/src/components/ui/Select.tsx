...
const Select = React.forwardRef<HTMLButtonElement, SelectProps>(({
    ...
    const getSelectedDisplay = () => {
        ...
        if (multiple) {
            const selectedOptions = options?.filter((opt: SelectOption) => Array.isArray(value) && value?.includes(opt?.value));
    ...
    const handleOptionSelect = (option: SelectOption) => {
        if (multiple) {
            const newValue = (Array.isArray(value) ? value : []) as (string | number)[];
